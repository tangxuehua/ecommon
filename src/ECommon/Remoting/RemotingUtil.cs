...
namespace ECommon.Remoting
{
    public class RemotingUtil
    {
        public static byte[] BuildRequestMessage(RemotingRequest request)
        {
            ...
            var message = new byte[13 + request.Body.Length];

            sequenceBytes.CopyTo(message, 0);
            codeBytes.CopyTo(message, 8);
            isOnewayBytes.CopyTo(message, 12);
            request.Body.CopyTo(message, 13);
        ...
        public static RemotingRequest ParseRequest(byte[] messageBuffer)
        {
            ...
            Array.Copy(messageBuffer, 8, codeBytes, 0, 4);
            Array.Copy(messageBuffer, 12, isOnewayBytes, 0, 1);
